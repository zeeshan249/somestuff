<template >
  ...
  <v-btn small  tile  color="green" icon>
          ...
          <student-view-details-dialog-component
            ...
            @onCertificateApprovedRejectedEvent="onCertificateApprovedRejected($event)"
...
export default {
  ...
  methods: {
    ...
    paginate(e) {
      ...
      axios
        .get(`web_get_all_certificate_request_generated_student_franchise_wise_list?page=${e.page}`, {
          params: {
...
onImageDownloadFailed()
{
    ...
    getUserProfileImage(imageName) {
     ...
     onCertificateApprovedRejected(approveRejectCertificateResponseData) {
      if (approveRejectCertificateResponseData.isServerError == true) {
        this.snackBarColor = "error";
        this.changeSnackBarMessage(this.$t('message.message_something_went_wrong'));
      } else {
        if (approveRejectCertificateResponseData.data.Result == "1") {
          this.snackBarColor = "success";
          this.changeSnackBarMessage( this.$t('message.message_certificate_approved'));
          this.componentUpdateKey+=1;

        } else if (
          approveRejectCertificateResponseData.data.Result == "3"
        ) {
          this.snackBarColor = "success";
          this.changeSnackBarMessage(this.$t('message.message_certificate_rejected'));
          this.componentUpdateKey+=1;

        } else if (
          approveRejectCertificateResponseData.data.Result == "2"
        ) {
          this.snackBarColor = "error";
          this.changeSnackBarMessage(this.$t('message.message_failed to_approve_certificate'));
        } else if (
          approveRejectCertificateResponseData.data.Result == "4"
        ) {
          this.snackBarColor = "error";
          this.changeSnackBarMessage(this.$t('message.message_failed to_reject_certificate'));
        }
      }
    },
    //End
