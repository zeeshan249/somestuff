      ...
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title  class="d-none d-lg-block">
              <small> <strong>{{$t("label.label_applied_executive")}}</strong></small>
          </v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      excelFileNameForDownload:  "AppliedExecutiveList"+moment().format('DD/MM/YYYY')+".xls",
  ...
  methods: {
    ...
    paginate(e) {
      ...
      axios
        .get(`web_get_all_applied_executive_list?page=${e.page}`, {
          params: {
    ...
    downloadAppliedFranchiceInPDF() {
      ...
      pdf.save("AppliedExecutiveList"+moment().format('DD/MM/YYYY')+".pdf");
    },
    //End
