      ...
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title  class="d-none d-lg-block">
           <small> <strong>{{$t("label.label_approved_executive")}}</strong></small>
          </v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      excelFileNameForDownload:  "ApprovedExecutiveList"+moment().format('DD/MM/YYYY')+".xls",
...
this.authorizationConfig = {
  ...
  methods: {
    ...
    paginate(e) {
      ...
      axios
        .get(`web_get_all_approved_executive_list?page=${e.page}`, {
          params: {
    ...
    downloadApprovedFranchiceInPDF() {
      ...
      pdf.save("ApprovedExecutiveList"+moment().format('DD/MM/YYYY')+".pdf");
    },
    //End
        ...
        .then(({ data }) => {
          ...
          if (data.update_flag == 1) {
            ...
            this.componentUpdateKey+=1
               ...
               this.snackBarColor="error"
this.changeSnackBarMessage(data.responseMessage,false);
          }
        })
        .catch(error => {
            this.snackBarColor="error"
