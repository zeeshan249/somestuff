<template >
  <div   :key="componentUpdateKey">
    ...
    <v-data-table
      ...
      :items="certificateListWithIndex"
      class="elevation-1"
      ...
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title  class="d-none d-lg-block">
              <small> <strong>{{$t("label.label_generated")}}</strong></small>
          </v-toolbar-title>
  ...
  <v-btn small  tile  color="green" icon>
              <download-excel
                :data="certificateListDataForExcelDownload"
                :fields="certificateFieldsForExcelDownload"
                :name="excelFileNameForDownload"
              > <v-icon>mdi-file-excel</v-icon></download-excel>
            </v-btn>
            <v-btn small  tile  color="red" @click="downloadCertificateDetailsInPDF" icon>
...
{
          text: this.$t("label.label_certificate_request_number"),
          align: "center",
          sortable: false,
          value: "certificate_application_number"
        },
        {
          text: this.$t("label.label_certificate_rejected_date"),
          align: "center",
          sortable: false,
          value: "certificate_approved_rejected_on"
        },
      ...
      // For Excel Download
      certificateFieldsForExcelDownload: {
        "Student Code": "student_code",
        "Registration Code": "student_registration_code",
        "Name": "student_name",
        "Mobile": "student_mobile",
        "Certificate Application Number": "certificate_application_number",
        "Rejected Date": "certificate_approved_rejected_on",
      },
      certificateListDataForExcelDownload: [],
      excelFileNameForDownload:  "CertificateRequestGeneratedList"+moment().format('DD/MM/YYYY')+".xls",

      // End

      // For PDF download
      certificateFieldsForPDFDownload: [
         "student_code",
        "student_registration_code",
         "student_name",
         "student_mobile",
        "certificate_application_number",
         "certificate_approved_rejected_on"
      ],
 ...
 //Snackbar Message
  ...
  computed: {
    ...
    certificateListWithIndex() {
      return this.certificateList.map((items, index) => ({
        ...items,
        index: index + 1
      }));
    }
  ...
  methods: {
    ...
    paginate(e) {
      ...
      axios
        .get(`web_get_rejected_certificate_franchise_wise_list?page=${e.page}`, {
          params: {
        ...
        .then(({ data }) => {
          this.tableDataLoading = false;
            ...
            this.certificateListDataForExcelDownload = data.data;
    ...
    // Download PDF
    downloadCertificateDetailsInPDF() {
      let headerConfig = this.certificateFieldsForPDFDownload.map(key => ({
        name: key,
        prompt: key,
        width: 100,
        align: "center",
      ...
      pdf.table(2, 3, this.certificateList, headerConfig);
      pdf.save("ActivecertificateList"+moment().format('DD/MM/YYYY')+".pdf");
    },
    //End
