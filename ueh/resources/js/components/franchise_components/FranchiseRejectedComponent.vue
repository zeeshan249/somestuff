      ...
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title class="d-none d-lg-block">
               <small> <strong>{{$t("label.label_rejected_franchise")}}</strong></small>
          </v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      excelFileNameForDownload:  "rejectedFranchiseList"+moment().format('DD/MM/YYYY')+".xls",
  ...
  methods: {
    ...
    paginate(e) {
      ...
      axios
        .get(`web_get_all_rejected_franchise_list?page=${e.page}`, {
          params: {
    ...
    downloadRejectedFranchiceInPDF() {
      ...
      pdf.save("rejectedFranchiseList"+moment().format('DD/MM/YYYY')+".pdf");
    },
    //End
