<template>
  ...
  <v-container>
    ...
    <v-row dense>
        ...
        <v-col class="d-flex flex-column ml-2">
          ...
          <v-card>
            <v-app-bar dark color="primary">
              <v-toolbar-title color="success">{{ $t('label_roles_and_permissions') }}</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-btn
              ...
              >
                <v-icon color="white" @click="isAddRoleCardVisible=!isAddRoleCardVisible">mdi-plus</v-icon>
              </v-btn>
            </v-app-bar>
            ...
            <v-container>
              <v-card class="mx-auto" max-width="100%">
                ...
                >
                  ...
                  <template v-slot:top>
                    <v-toolbar flat>
                      <v-toolbar-title>{{$t('label_edit_roles')}}</v-toolbar-title>
                      <v-spacer></v-spacer>
                  ...
                  <template v-slot:item.actions="{ item }">
                    ...
                    <v-icon v-if="item.is_role_active=='Active'"
                      v-permission="'Edit Role'"
                      small
                      color="error"
                      @click="disableRole(item)"
                    >mdi-delete</v-icon>
                    <v-icon v-if="item.is_role_active=='Inactive'"
                      v-permission="'Edit Role'"
                      small
                      color="success"
                      @click="disableRole(item)"
      ...
      <transition name="fade" mode="out-in">
        <v-col class="d-flex flex-column mr-2" v-if="isAddRoleCardVisible">
          <v-card>
            <v-app-bar dark color="primary">
              <v-toolbar-title color="success">{{$t('label_roles')}}</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-btn icon class="ma-2" outlined small color="white">
                <v-icon color="white" @click="isAddRoleCardVisible=!isAddRoleCardVisible">mdi-minus</v-icon>
              </v-btn>
            </v-app-bar>

            <v-container>
              <v-form ref="saveRoleForm" v-model="isSaveRoleFormValid" lazy-validation>
                <v-row dense>
                  ...
                  <v-col cols="12" xs="12" sm="12" md="4">
                    <v-btn
                      ...
                      tile
                      color="primary"
                      :disabled="!isSaveRoleFormValid || isSaveRoleFormDataProcessing"
                      @click="saveRole"
                    >{{isSaveRoleFormDataProcessing==true?$t("label_processing") : $t("label_save") }}</v-btn>
                  </v-col>
                </v-row>
              </v-form>
            </v-container>
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        { text: this.$t("label_role_name"), value: "name", width: "50%" },
        {
  ...
  methods: {
    ...
    disableRole(item, $event) {
      let userDecision = confirm(this.$t("label_are_you_sure_change_status_role"));
      if (userDecision) {
        ...
        this.$http
          .post(
            "web_update_role",
            {
              ...
              roleName: item.name,
              roleId: item.id,
              isRoleActive: item.is_role_active=='Active'?0:1
            },
            this.authorizationConfig
          )
          .then(({ data }) => {
            ...
            //User Unauthorized
             if (data.error == "Unauthorized" || data.permissionError=="Unauthorized") {
              ...
              // Role Exists
                ...
                this.changeSnackBarMessage(this.$t("label_role_exists"));
              }
              // Role Saved
              else if (data.responseData == 2) {
                this.roleName = "";
                this.snackBarColor = "success";
                this.changeSnackBarMessage(this.$t("label_role_saved"));
                this.getAllRole(event);
              ...
              // Role Updated
                ...
                this.snackBarColor = "success";
                this.changeSnackBarMessage(this.$t("label_role_name_updated"));
                this.getAllRole(event);
                this.isSaveEditClicked = 1;
                this.roleName = "";
              }
