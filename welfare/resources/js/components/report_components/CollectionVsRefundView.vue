        ...
        <v-col class="d-flex flex-column ml-2">
          ...
          <v-card outlined="">
            <v-app-bar dark color="primary">
              <v-toolbar-title color="success">Collection And Refund Report - Notary</v-toolbar-title>
              <v-spacer></v-spacer>
            ...
            <v-container>
                ...
                <v-row dense>
                    <v-col cols="12" md="4">
                        <v-btn  class="ml-2" dense rounded color="primary" @click="param=1; getAllCollectionAndRefund($event,param);" :disabled="tableDataLoading" >Details Report</v-btn>
                    </v-col>


                     <v-col cols="12" md="4">
                        <v-btn  class="ml-2" dense rounded color="primary" @click="param=2; getAllCollectionAndRefund($event,param);" :disabled="tableDataLoading" >Head Wise</v-btn>
                    </v-col>


                     <v-col cols="12" md="4">
                        <v-btn  class="ml-2" dense rounded color="primary" @click="param=3; getAllCollectionAndRefund($event,param);" :disabled="tableDataLoading" >Advocate Wise</v-btn>
                    </v-col>
                </v-row>
        ...
        <v-col class="d-flex flex-column ml-2">
            ...
            <!-- Collection Entry Details Start -->
                ...
                <v-data-table
                  ...
                  :items-per-page="50"
                  @pagination="getAllCollectionAndRefund(param)"
                  :footer-props="{
                ...
                >
                  ...
                  <template v-slot:top>
                    <v-toolbar flat>
                      <v-toolbar-title>Collection And Refund Report - Notary</v-toolbar-title>
                      <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    ...
    // Datatable Start
      ...
      excelData: [],
      excelFileName:
        "DailyCollectionVsRefund" + "_" + moment().format("DD/MM/YYYY") + ".xls"
    };
  },
  computed: {
  ...
  created() {
    ...
    // Get all collection
    //this.getAllCollectionAndRefund(event,'1');
  },
  ...
  methods: {
    ...
    getAllCollectionAndRefund(e,param) {

if(param == '1')
{
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_all_collection_and_refund_receiptwise?page=${e.page}`, {
          params: {
     ...
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_head_wise_collection_and_refund?page=${e.page}`, {
          params: {
            centerId: ls.get("loggedUserCenterId"),
            perPage: e.itemsPerPage,
             selectedStartDate : this.selectedStartDate,
     ...
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_advocate_wise_collection_and_refund?page=${e.page}`, {
          params: {
            centerId: ls.get("loggedUserCenterId"),
            perPage: e.itemsPerPage,
             selectedStartDate : this.selectedStartDate,
