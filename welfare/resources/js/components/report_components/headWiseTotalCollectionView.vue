        ...
        <v-col class="d-flex flex-column ml-2">
          ...
          <v-card outlined="">
            <v-app-bar dark color="primary">
              <v-toolbar-title color="success">Daily Refund</v-toolbar-title>
              <v-spacer></v-spacer>
            ...
            <v-container>
                ...
                <v-col cols="12" md="4">
                  <v-dialog ref="dialogEndDate" v-model="modalEndDate" :return-value.sync="selectedEndDate" width="290px" >
                    ...
                    <v-col cols="12" md="4">
                        <v-btn  class="ml-2" dense rounded color="primary" @click="param=1; getAllCollection($event,param);" :disabled="tableDataLoading" >Details Report</v-btn>
                    </v-col>
...
export default {
  ...
  data() {
    ...
    // Datatable Start
      ...
      excelData: [],
      excelFileName:
        "DailyRefundAsExcel" + "_" + moment().format("DD/MM/YYYY") + ".xls"
    };
  },
  computed: {
...
if(param == '1')
{
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_all_refund_datewise?page=${e.page}`, {
          params: {
     ...
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_all_refund_datewise_headwise?page=${e.page}`, {
          params: {
            centerId: ls.get("loggedUserCenterId"),
            perPage: e.itemsPerPage,
             selectedStartDate : this.selectedStartDate,
     ...
     this.tableDataLoading = true;
      this.$http
        .get(`web_get_all_refund_datewise_advocatewise?page=${e.page}`, {
          params: {
            centerId: ls.get("loggedUserCenterId"),
            perPage: e.itemsPerPage,
             selectedStartDate : this.selectedStartDate,
    ...
    savePDF() {
      ...
      pdfDoc.save(
        "DailyRefundAsPDF" + "_" + moment().format("DD/MM/YYYY") + ".pdf"
      );
    }
  }
};
