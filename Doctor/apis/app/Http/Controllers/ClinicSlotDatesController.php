...
//        }
    ...
    public function  saveSlotDate(Request $request)
    {
		...
		$chk_no_opd=DB::table('dm_no_opd_ot')
             ->where('dm_no_opd_ot.doctor_id',$request->doctor_id)
            ->where('dm_no_opd_ot.no_opd_date',$request->in_clinic_slot_dates)
            ->where('status',1)->count();
         

        if($chk_no_opd > 0)
        {
             return response()->json(['message' => 'You have added No OTP/OT on this date.','result'=>'error'], 200);
        }
        else
        {
